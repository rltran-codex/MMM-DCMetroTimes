...
module.exports = NodeHelper.create({
    ...
    if (notification === "REGISTER_CONFIG") {
      ...
      if (theConfig.showIncidents) {
        ...
        setInterval(function () {
          self.updateIncidents(theConfig);
      ...
      if (theConfig.showStationTrainTimes) {
        ...
        setTimeout(function () {
          self.updateTrainTimes(theConfig);
        }, 1000);
        setInterval(function () {
          self.updateTrainTimes(theConfig);
  ...
  getStationName: function (theStationCode) {
    ...
    for (
      var cIndex = 0;
      cIndex < this.stationInformationList.length;
      cIndex++
    ) {
  ...
  doesNotContainExcludedDestination: function (
    theConfig,
    theStationCode,
    theDestinationCode
  ) {
    ...
    for (
      var cIndex = 0;
      cIndex < theConfig.destinationsToExcludeList.length;
      cIndex++
    ) {
  ...
  getTrainQuery: function (theConfig) {
    ...
    for (
      var cIndex = 0;
      cIndex < theConfig.stationsToShowList.length;
      cIndex++
    ) {
  ...
  parseTrainTimes: function (theConfig, theTrains) {
    ...
    for (var cIndex = 0; cIndex < theTrains.length; cIndex++) {
      ...
      if (train.DestinationCode !== null) {
        ...
        // it to the list
        if (
          this.doesNotContainExcludedDestination(
            theConfig,
            tLocationCode,
            tDestinationCode
          ) &&
  ...
  isWithinConfigThreshold: function (theConfig, theMin) {
    if (
      theMin < theConfig.hideBusTimesLessThan ||
  ...
  doesNotContainExcludedRoute: function (theConfig, theStopID, theRouteID) {
    ...
    for (
      var cJindex = 0;
      cJindex < theConfig.stopsToShowList.length;
      cJindex++
    ) {
      if (theConfig.stopsToShowList[cJindex] == theStopID) {
        if (theConfig.routesToExcludeList.length != 0) {
          ...
          for (
            var cIndex = 0;
            cIndex < theConfig.routesToExcludeList[cJindex].length;
            cIndex++
          ) {
  ...
  parseBusTimes: function (theConfig, stopID, responseContent, busStopList) {
    ...
    for (var cIndex = 0; cIndex < theBuses.length; cIndex++) {
      ...
      if (bus.RouteID !== null) {
        ...
        // it to the list
        if (
          this.doesNotContainExcludedRoute(theConfig, stopID, bRouteID) &&
    ...
    if (
      theConfig.stopsToShowList[theConfig.stopsToShowList.length - 1] == stopID
    ) {
      // return payload is the module id and the station train list
      var returnPayload = {
        identifier: theConfig.identifier,
